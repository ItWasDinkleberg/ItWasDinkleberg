...
' .hdr { font-family: Courier, monospace; font-size: 16px; fill: #3fb950; font-weight:
bold; }',
...
svg.append(f'<text x="{x_offset}" y="{y_offset}" class="txt">{display}: <tspan
class="val">{level}</tspan></text>')
col += 1
if col >= 4:
col = 0
y_offset += row_height
...
svg.append(f'<line x1="20" y1="{y_offset + 10}" x2="480" y2="{y_offset + 10}"
stroke="#30363d" stroke-width="1" />')
svg.append(f'<text x="20" y="{y_offset + 35}" class="hdr" fill="#58a6ff">&gt;
SYS.TOTAL_LVL : {total}</text>')
svg.append('</svg>')
